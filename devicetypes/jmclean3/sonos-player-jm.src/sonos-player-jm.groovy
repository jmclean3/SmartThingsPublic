...
metadata {
	definition (name: "Sonos Player JM", namespace: "jmclean3", author: "JMCLEAN") {
		capability "Actuator"
		capability "Switch"
		capability "Refresh"
		capability "Sensor"
	...
	tiles(scale: 2) {
		multiAttributeTile(name: "main", type:"mediaPlayer", width:6, height:4, canChangeIcon: true) {
			...
			tileAttribute ("device.level", key: "SLIDER_CONTROL") {
				attributeState("level", action:"tileSetLevel")
			}
			...
			if (!msg.body) {
				...
				if (node.size()) {
					...
					if (myNode != null) {
						...
						if (coordinator != null && myCoordinator != myNode.'@UUID') {
							...
							if(coordinatorDevice!=null){
							   ...
							   sendEvent(name: "switch", value: "off", displayed: false)
							   	...
							   	log.trace"Cannot get child device"
							   }
						}
						else {
							// Not grouped
						...
						log.trace "Cannot update group change because no zone group member found"
					}
				...
				if (node.size()) {
					...
					if (trackUri.contains("//s3.amazonaws.com/smartapp-")) {
						...
						if (trackMeta || transportMeta) {
							...
							def stationMetaXml = null
							try {
								stationMetaXml = metaData ? parseXml(metaData) : null
							} catch (Exception e) {
								log.debug("metaData is not an XML file")
							}
